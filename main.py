...
import logging
logging.basicConfig(
    level=logging.ERROR,  # Pode ser DEBUG, INFO, WARNING, ERROR, CRITICAL
...
if args.benchmark:
    ...
    if args.graph_generator == 'embedding':
        ...
        graph_data.name = args.graph_generator
    ...
    visualize_attributes(graph_data.x.detach().numpy(), graph_data.y.tolist(), class_colors, save_path=f'logs/{data_execucao}/figures/{graph_data.name}_features.png')

    # Graph metrics visualization stage
    if graph_data.edge_index != None: 
        g = edge_index_to_igraph(graph_data.edge_index, num_nodes = len(graph_data.y))
        

        plot_igraph_community(g, graph_data.y.tolist(), class_colors, output_file = f'logs/{data_execucao}/figures/{graph_data.name}_graph.png')

        general_degree_distribution(g, graph_data.y.tolist(), class_colors, save_path = f'logs/{data_execucao}/figures/{graph_data.name}_node_degree_distribution.png')