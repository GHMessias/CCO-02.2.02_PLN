...
df.rename(columns = {'news':'old_news'}, inplace=True)

df['news'] = df.apply(lambda row: llm_query(row['old_news'], 'prompt_data_clearning.txt'), axis = 1)

# df_fake['Model Answer'] = df_fake.progress_apply(lambda row: llm_query(row['old_news'], args.prompt_debias_input_path), axis=1)
# df_true['Model Answer'] = df_true.progress_apply(lambda row: llm_query(row['old_news'], args.prompt_summarization_input_path), axis=1)

# df_final = pd.concat([df_fake, df_true], ignore_index = True)
df[['news', 'label']].to_csv(f'dataset/llm_clean_data.tsv', sep = '\t')
